...
// newSignedJSONCredential creates a JSON credential with proof, without hitting the network
// (verification method key is pinned so no DID resolution is needed).
...
// newSignedJWTCredential creates a signed JWT credential, without hitting the network.
