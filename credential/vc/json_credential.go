...
func (e *JSONCredential) AddCustomProof(proof *dto.Proof, opts ...CredentialOpt) error {
	...
	prev := cloneJSONValue(e.credentialData["proof"])
	if err := (*jsonmap.JSONMap)(&e.credentialData).AddCustomProof(proof); err != nil {
		return err
	}
	if err := e.executeOptions(opts...); err != nil {
		if prev == nil {
			delete(e.credentialData, "proof")
		} else {
			e.credentialData["proof"] = prev
		}
		return err
	}
	return nil
